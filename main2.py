...
def valid(model, valid_dataloader, device, epoch, reduction):
    ...
    # Check if true_labels are continuous (regression) or binary (classification)
    # For continuous targets like Y_COST, use different metrics
    is_binary_task = len(np.unique(true_labels)) <= 10 and np.all(np.isin(true_labels, [0, 1]))
    
    if not is_binary_task:
        # For regression tasks (e.g., Y_COST), use MSE-based metrics
        logger.warning("Detected continuous target variable. Using regression metrics instead of uplift classification metrics.")
        # Calculate a simple metric: mean uplift for treated vs control
        treated_mask = is_treatment == 1
        control_mask = is_treatment == 0
        
        if np.any(treated_mask) and np.any(control_mask):
            treated_mean = np.mean(true_labels[treated_mask])
            control_mean = np.mean(true_labels[control_mask])
            ate = treated_mean - control_mean  # Average Treatment Effect
            
            # MSE of predictions
            mse = np.mean((predictions - (true_labels * is_treatment - true_labels * (1 - is_treatment)))**2)
            
            valid_result = {'ATE': ate, 'MSE': mse, 'treated_mean': treated_mean, 'control_mean': control_mean}
        else:
            valid_result = {'error': 'Not enough samples in treated or control group'}
    else:
        # Original binary classification metrics
        if args.mtask:
    ...
    for epoch in range(start_epoch, num_epoch):
        ...
        if best_valid_metrics is None:
            if isinstance(valid_metrics, list):
                ...
                # Initialize based on what metrics are present
                if 'QINI' in valid_metrics:
                    best_valid_metrics = {'QINI': -10, 'AUUC': -10, 'WAU': -10, 'u_at_k': -10}
                elif 'ATE' in valid_metrics:
                    # For regression tasks, lower MSE is better (use high initial value)
                    best_valid_metrics = {'ATE': 0, 'MSE': float('inf'), 'treated_mean': 0, 'control_mean': 0}
                    # Update metric_names for regression tasks
                    metric_names = ['MSE']
                else:
...
if __name__ == "__main__":
    ...
    # 1. 指向你的 HDF5 数据文件
    file_paths = [f'data/h5_data/train_part_{i+1}.h5' for i in range(10)]
    ...
    for fold_idx, (train_files, test_files) in fold_enumerator:
        ...
        if ave_best_valid_metrics is None:
            if isinstance(best_valid_metrics, list):
                ...
                # Initialize based on what metrics are present in best_valid_metrics
                if 'QINI' in best_valid_metrics:
                    ave_best_valid_metrics = {'QINI': 0., 'AUUC': 0., 'WAU': 0., 'u_at_k': 0.}
                elif 'ATE' in best_valid_metrics:
                    ave_best_valid_metrics = {'ATE': 0., 'MSE': 0., 'treated_mean': 0., 'control_mean': 0.}
                else:
                    ave_best_valid_metrics = {k: 0. for k in best_valid_metrics}
